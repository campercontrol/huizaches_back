...
def reset_password( 
    ...
    if not exist_email:
        ...
        return {"mensaje": "Se ha enviado un correo con instrucciones para la recuperacion de su contraseña", "data": ""}


@user_routes.post("/usuario/change_password/{email}", tags=["Usuarios"])
def change_password(email:str,change_pass:UserChangePassword,response: Response, db: Session = Depends(get_db)):
    ...
    if change_pass.password != change_pass.password_confirm:
        ...
        if respuesta_update_user != 0:
            response.status_code = 200
            return {"mensaje": "La contraseña se ha cambiado correctamente, lo proxima vez que inicie sesion podra usar su nueva contraseña", "data": []}
        else:
            response.status_code = 401
